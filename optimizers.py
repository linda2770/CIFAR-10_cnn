...
class SGDmomentumOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        # Momentum update
        for k in grads:
            velocitys[k] = momentum * velocitys[k] - learning_rate * grads[k] # integrate velocity
            params[k] += velocitys[k] # integrate position
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            mt = momentums[k] / (1-beta1**t)
            velocitys[k] = beta2*velocitys[k] + (1-beta2)*(grads[k]**2)
            vt = velocitys[k] / (1-beta2**t)
            params[k] += - learning_rate*mt / (np.sqrt(vt) + eps)
            t += 1
        #raise NotImplementedError
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            velocitys[k] = beta2*velocitys[k] + (1-beta2)*(grads[k]**2)
            vt = velocitys[k] / (1-beta2**t)
            params[k] += - learning_rate*(beta1*mt + (1-beta1)*grads[k]/(1-beta1**t))/ (np.sqrt(vt) + eps)
            t += 1
        #raise NotImplementedError