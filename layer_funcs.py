...
def conv2d_forward(x, w, b, pad, stride):
    ...
    Z = np.zeros((batch,new_height, new_width, num_of_filters))
    ...
    for i in range(batch):               # loop over the batch of training examples
        #x_pad = x_pad[i]                 # select i th training example's padded activation
        for h in range(new_height):      # loop over the vertical axis of the output volume
            for j in range(new_width):   # loop over the horizontal axis of the output volume
                for k in range(num_of_filters):  # loop over the channels of the output volume
                    ...
                    Z[i,h,j,k] = np.sum(x_pad[i, vertical_start:vertical_end,horizontal_start:horizontal_end,:]*w[k,:,:,:]) + b[k]
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    # Initialize dx_prev, dW, db with the correct shapes
    dx_prev = np.zeros((batch, height, width, channels))                           
    dW = np.zeros((num_of_filters, filter_height, filter_width, channels))
    db = np.zeros((1, 1, 1,  channels))
    
    # Pad x_prev and dx_prev
    x_prev_pad = np.pad(x, ((0, 0), (pad, pad), (pad, pad), (0, 0)), 'constant', constant_values=0)
    dx_prev_pad = np.pad(dx_prev, ((0, 0), (pad, pad), (pad, pad), (0, 0)), 'constant', constant_values=0)
    for i in range(batch):               # loop over the batch of training examples
        #x_pad = x_pad[i]                 # select i th training example's padded activation
        for h in range(height_new):      # loop over the vertical axis of the output volume
            for j in range(width_new):   # loop over the horizontal axis of the output volume
                for k in range(num_of_filters):  # loop over the channels of the output volume
                    vertical_start = h
                    vertical_end = vertical_start + filter_height
                    horizontal_start = j
                    ...
                    a_slice =x_prev_pad[i, vertical_start:vertical_end,horizontal_start:horizontal_end,:]
                    #dx_prev_pad[i,vertical_start:vertical_end,horizontal_start:horizontal_end, :] += w[k,:,:,:] * d_top[i, h, j, k]
                    dW[k,:,:,:] += a_slice * d_top[i, h, j, k]
...
def max_pool_forward(x, pool_size, stride):
    ...
    new_height = int(1+(height - pool_size) // stride)
    new_width = int(1+(width - pool_size) // stride)
    
    # Initialize output matrix A
    A = np.zeros((batch, new_height, new_width,channels)) 
    
    for i in range(batch):               # loop over the batch of training examples
        #                 # select i th training example's padded activation
        for h in range(new_height):      # loop over the vertical axis of the output volume
            for j in range(new_width):   # loop over the horizontal axis of the output volume
                for k in range(channels):  # loop over the channels of the output volume
                    ...
                    A[i,h,j,k] = np.max(x[i, vertical_start:vertical_end,horizontal_start:horizontal_end,k])
    return A                             
...
def max_pool_backward(dout, x, pool_size, stride):
    ...
    # Initialize dx_prev with the correct shapes
    dx_prev = np.zeros((batch, height, width, channels))                           
    
   
    for i in range(batch):               # loop over the batch of training examples
        #X = x[i]                 # select i th training example's padded activation
        for h in range(height_new):      # loop over the vertical axis of the output volume
            for j in range(width_new):   # loop over the horizontal axis of the output volume
                for k in range(channels):  # loop over the channels of the output volume
                    vertical_start = h
                    ...
                    a_prev_slice = x[i,vertical_start:vertical_end,horizontal_start:horizontal_end,k]
                    mask = a_prev_slice == np.max(a_prev_slice)
                    dx_prev[i, vertical_start:vertical_end,horizontal_start:horizontal_end, k] += np.multiply(mask, dout[i, h, j, k])
